...
/*
$sql = mysql_query("SELECT estado_inst,COUNT(estado_inst) AS total FROM becas_estudiantes 
	WHERE estado_inst NOT LIKE '' GROUP BY estado_inst");
$sql2 =mysql_query("SELECT COUNT(*) as inti FROM becas_estudiantes");
...
$graph2 = new PieGraph(450,300,"auto");
$graph2->SetShadow();

// Set A title for the plot
$graph2->title->Set("Correos Mensuales");
$graph2->title->SetFont(FF_FONT1,FS_BOLD);

// Create
$sql_sexo = mysql_query("SELECT MONTHNAME(date_crea_adh),count(*) as total_es, count(*) as total 
	FROM correfax.gac_correspondencia WHERE 
date_crea_adh not like '' group by date_crea_adh") ;

while($row2 = mysql_fetch_array($sql_sexo))
{
$data2[] = $row2['total'];
$tado[] = $row2['MONTHNAME(date_crea_adh)'];
$t_es[] = $row2['total_es'];
}
...
<TR><td><div><IMG src="graphico_02.jpg" width="450" height="300" align="bottom" border="0"></div></TD></TR>
</table>
	</div>
